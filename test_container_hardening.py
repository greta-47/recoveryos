...
def test_docker_build():
    ...
    try:
        result = subprocess.run(
            ["docker", "build", "--target", "runtime", "-t", "recoveryos:hardened-test", "."],
            capture_output=True,
            text=True,
            cwd="/home/ubuntu/repos/recoveryos",
        )
    ...
    try:
        result = subprocess.run(
            [
                ...
                "--read-only",
                "--tmpfs",
                "/tmp/app:size=100M,noexec,nosuid,nodev",
                "--tmpfs",
                "/var/log/app:size=50M,noexec,nosuid,nodev",
                "--tmpfs",
                "/var/cache/app:size=50M,noexec,nosuid,nodev",
                "-e",
                "READONLY_MODE=true",
                "-p",
                "8001:8000",
                "--name",
                "recoveryos-readonly-test",
        ...
        if result.returncode == 0:
            ...
            health_result = subprocess.run(
                ["curl", "-f", "http://localhost:8001/healthz"], capture_output=True, text=True
            )
...
def test_no_build_tools():
    ...
    try:
        result = subprocess.run(
            ["docker", "run", "--rm", "recoveryos:hardened-test", "sh", "-c", 'which gcc || echo "gcc not found"'],
            capture_output=True,
            text=True,
        )
        ...
        result = subprocess.run(
            [
                ...
                "sh",
                "-c",
                'dpkg -l | grep -E "(build-essential|libpq-dev)" || echo "dev packages not found"',
            ],
            capture_output=True,
            text=True,
        )
...
def test_non_root_user():
    ...
    try:
        result = subprocess.run(
            ["docker", "run", "--rm", "recoveryos:hardened-test", "id"], capture_output=True, text=True
        )
...
def test_security_labels():
    ...
    try:
        result = subprocess.run(["docker", "inspect", "recoveryos:hardened-test"], capture_output=True, text=True)
