...
class PrivacyBudget:
    ...
    def can_spend(self, epsilon: float, delta: float = 0.0) -> bool:
        return (self.used_epsilon + epsilon <= self.epsilon and 
                self.used_delta + delta <= self.delta)
...
class DifferentialPrivacyMechanism:
    ...
    def sparse_vector_technique(self, queries: List[Callable], threshold: float, 
                               max_responses: int = 1) -> List[Optional[float]]:
...
class ClinicalPrivacyProtector:
    ...
    def privatize_emotion_analysis(self, emotion_scores: Dict[str, float], 
                                 user_id: str, epsilon: float = 0.05) -> Dict[str, float]:
    ...
    def privatize_risk_assessment(self, risk_factors: List[Dict[str, Any]], 
                                user_id: str, epsilon: float = 0.1) -> List[Dict[str, Any]]:
        budget = self.get_user_budget(user_id)
        
        if not budget.can_spend(epsilon):
            logger.warning(f"Insufficient privacy budget | User={user_id}")
            return [{"name": "privacy_protected", "score": 0.5, "explanation": "Privacy budget exceeded"}]
    ...
    def privatize_clinical_insights(self, insights: List[str], user_id: str, 
                                  epsilon: float = 0.08) -> List[str]:
        budget = self.get_user_budget(user_id)
        ...
        def utility_fn(insight: str) -> float:
            therapeutic_keywords = ["recovery", "coping", "support", "healing", "progress"]
...
class PrivacyAudit:
    ...
    def generate_privacy_report(self) -> Dict[str, Any]:
        ...
        global_budget_used = (
            ...
            self.protector.global_budget.epsilon * 100
        )
    ...
    def validate_privacy_compliance(self) -> Dict[str, Any]:
        ...
        high_risk_users = [
            user_id for user_id, budget in self.protector.user_budgets.items()
...
def apply_differential_privacy(data: Union[Dict, List, float], 
                             epsilon: float = 0.1, 
                             mechanism: PrivacyMechanism = PrivacyMechanism.LAPLACE) -> Any:
