...
- **Deterministic lockfile** on **Linux + Python 3.11** only (never on Mac):
  ```bash
  docker run --rm -v "$PWD":/app -w /app python:3.11-slim bash -lc   'pip install -U pip pip-tools && pip-compile --generate-hashes -o requirements.lock.txt requirements.txt'
  ```
