...
class RecoveryCausalGraph:
    ...
    def _initialize_recovery_graph(self):
        recovery_nodes = [
            CausalNode("stress_level", "psychological", "Current stress and anxiety levels", True, True),
            CausalNode("social_support", "social", "Quality and quantity of social connections", True, True),
            CausalNode("coping_skills", "behavioral", "Availability and use of healthy coping mechanisms", True, True),
            CausalNode("substance_craving", "physiological", "Intensity of substance use urges", True, False),
            CausalNode("mood_state", "psychological", "Current emotional and mood state", True, True),
            CausalNode("sleep_quality", "physiological", "Sleep duration and quality", True, True),
            CausalNode("relapse_risk", "outcome", "Probability of substance use relapse", True, False),
            CausalNode("trauma_triggers", "psychological", "Exposure to trauma-related triggers", True, True),
            CausalNode("medication_adherence", "behavioral", "Consistency with prescribed medications", True, True),
            CausalNode("therapy_engagement", "behavioral", "Active participation in therapeutic activities", True, True),
            CausalNode("employment_status", "social", "Work and financial stability", True, True),
            CausalNode("physical_health", "physiological", "Overall physical health and wellness", True, True)
        ]
        ...
        recovery_edges = [
            CausalEdge("stress_level", "substance_craving", CausalRelationType.DIRECT_CAUSE, 0.7, 0.85, ["clinical_studies", "patient_reports"]),
            CausalEdge("social_support", "stress_level", CausalRelationType.DIRECT_CAUSE, -0.6, 0.80, ["social_psychology", "recovery_research"]),
            CausalEdge("coping_skills", "stress_level", CausalRelationType.DIRECT_CAUSE, -0.8, 0.90, ["cbt_research", "clinical_trials"]),
            CausalEdge("substance_craving", "relapse_risk", CausalRelationType.DIRECT_CAUSE, 0.9, 0.95, ["addiction_research", "longitudinal_studies"]),
            CausalEdge("mood_state", "substance_craving", CausalRelationType.DIRECT_CAUSE, 0.5, 0.75, ["psychiatric_research"]),
            CausalEdge("sleep_quality", "mood_state", CausalRelationType.DIRECT_CAUSE, 0.6, 0.80, ["sleep_research", "mental_health"]),
            CausalEdge("trauma_triggers", "stress_level", CausalRelationType.DIRECT_CAUSE, 0.8, 0.85, ["trauma_research", "ptsd_studies"]),
            CausalEdge("medication_adherence", "mood_state", CausalRelationType.DIRECT_CAUSE, 0.4, 0.70, ["pharmacological_studies"]),
            CausalEdge("therapy_engagement", "coping_skills", CausalRelationType.DIRECT_CAUSE, 0.7, 0.85, ["therapy_research"]),
            CausalEdge("employment_status", "stress_level", CausalRelationType.INDIRECT_CAUSE, -0.3, 0.65, ["socioeconomic_research"]),
            CausalEdge("physical_health", "mood_state", CausalRelationType.INDIRECT_CAUSE, 0.4, 0.70, ["health_psychology"]),
            CausalEdge("social_support", "therapy_engagement", CausalRelationType.MODERATOR, 0.3, 0.60, ["social_research"])
        ]
...
class CounterfactualReasoning:
    ...
    def generate_counterfactual(self, observed_state: Dict[str, float], 
                              intervention: Dict[str, float]) -> Dict[str, Any]:
...
class CausalInferenceEngine:
    ...
    def suggest_interventions(self, user_state: Dict[str, float], 
    ...
    def what_if_analysis(self, user_state: Dict[str, float], 
                        hypothetical_changes: Dict[str, float]) -> Dict[str, Any]:
