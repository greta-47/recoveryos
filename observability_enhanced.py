...
REQUEST_COUNT = Counter('recoveryos_requests_total', 'Total requests', ['endpoint', 'version', 'env', 'status'])
REQUEST_LATENCY = Histogram('recoveryos_request_duration_seconds', 'Request latency', ['endpoint', 'version', 'env'])
ERROR_RATE = Counter('recoveryos_errors_total', 'Total errors', ['endpoint', 'version', 'env', 'error_type'])
...
class PIIRedactor:
    ...
    PII_PATTERNS = [
        ...
        (r'\b\d{1,5}\s\w+\s(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd)\b', '[ADDRESS_REDACTED]'),
        (r'\b(?:patient|client)[-_]?id[-_]?\d+\b', '[PATIENT_ID_REDACTED]'),
        (r'\b(?:medical|record|chart)[-_]?(?:number|num|id)[-_]?\d+\b', '[MEDICAL_RECORD_REDACTED]'),
        (r'\b\d{3}-\d{3}-\d{4}\b', '[PHONE_REDACTED]'),
        (r'\b(?:dob|birth)[-_]?date[-_]?\d{1,2}[/-]\d{1,2}[/-]\d{2,4}\b', '[DOB_REDACTED]')
    ]
    ...
    def redact_dict(cls, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, str):
                ...
                redacted[key] = [cls.redact_dict(item) if isinstance(item, dict) else cls.redact_pii(str(item)) for item in value]
            else:
                redacted[key] = value
        return redacted
...
class EnhancedObservability:
    ...
    def track_request(self, endpoint_name: str, latency_ms: float, success: bool, 
                     request_data: Dict[str, Any] = None, error_type: str = None,
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            "endpoints": {name: {
                ...
                "avg_latency_ms": round(metric.total_latency_ms / metric.request_count if metric.request_count > 0 else 0, 2),
            ...
            "avg_success_rate": sum(m.success_rate for m in self.metrics.values()) / len(self.metrics) if self.metrics else 1.0,
            "version": self.version,
            "environment": self.environment,
            "timestamp": datetime.utcnow().isoformat() + "Z"
        }
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                enhanced_observability.track_request(
                    endpoint_name, latency_ms, success, request_data, error_type, correlation_id
                )
