...
class LoadTester:
    ...
    async def test_endpoint(self, session: aiohttp.ClientSession, endpoint: str, payload: Dict[str, Any], duration_minutes: int = 2):
    ...
    def generate_report(self):
        ...
        report = {
            ...
            "overall_summary": {
                ...
                "endpoints_meeting_sla": sum(1 for r in self.results.values() 
                                           if r["sla_compliance"]["p95_under_500ms"] and 
                                              r["sla_compliance"]["error_rate_under_1pct"]),
                "avg_error_rate": round(statistics.mean([r["error_rate_percent"] for r in self.results.values()]), 2),
                "avg_p95_latency": round(statistics.mean([r["latency_ms"]["p95"] for r in self.results.values()]), 2)
            }
        }
...
async def main():
    ...
    all_compliant = report['overall_summary']['endpoints_meeting_sla'] == report['overall_summary']['total_endpoints_tested']
