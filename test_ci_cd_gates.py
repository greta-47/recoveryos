...
def test_security_scanning():
    ...
    success, stdout, stderr = run_command(
        "pip-audit --desc --format=json --output=test-audit.json", check=False
    )
    if success:
    ...
    success, stdout, stderr = run_command(
        "bandit -r . -f json -o test-bandit.json", check=False
    )
    ...
    success, stdout, stderr = run_command(
        "safety check --json --output=test-safety.json", check=False
    )
    if success:
        print("✅ safety: PASS")
    else:
...
def main():
    ...
    run_command(
        "pip install pytest pytest-cov black ruff bandit pip-audit safety", check=False
    )
